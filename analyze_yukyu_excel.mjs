...
const filePath = join(__dirname, '有給休暇管理.xlsm');
  ...
  workbook.SheetNames.forEach(sheetName => {
    ...
    for (let i = 0; i < Math.min(10, rawData.length); i++) {
      ...
      if (row && row.some(cell => cell !== '')) {
        // Mostrar columnas A-T y sus valores
        const preview = row.slice(0, 20).map((cell, idx) => {
          const colLetter = XLSX.utils.encode_col(idx);
          const cellVal = cell === '' ? '(vacío)' : String(cell).substring(0, 12);
    ...
    // Verificar estructura específica del usuario
